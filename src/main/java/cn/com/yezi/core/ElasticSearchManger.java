...
public class ElasticSearchManger implements InitializingBean{
    ...
    public void init() {
        settings = Settings.settingsBuilder().put("cluster.name", "my.elk").put("client.transport.sniff", true)
                .build();
        try {
 
            client = TransportClient.builder().settings(settings).build()
                    .addTransportAddress(new InetSocketTransportAddress(InetAddress.getByName("192.168.138.128"), 9300))
                    .addTransportAddress(new InetSocketTransportAddress(InetAddress.getByName("192.168.138.128"), 8300));
